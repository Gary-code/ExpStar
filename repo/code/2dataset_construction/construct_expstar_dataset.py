...
def setup_logging():
    ...
    log_file = os.path.join(log_dir, f"create_finetune_dataset_with_ret_{timestamp}.log")
...
def create_fine_tuning_dataset(source_subject, source_unit, 
                              ...
                              output_base_dir="data/exp_star_data"):
    """
    Create fine-tuning dataset from source JSON files with detailed annotations
    Generate two rounds of dialogue if safety or principle information exists
    ...
    for exp_dir in experiment_dirs:
        # Get all JSON files in experiment directory
        ...
        for json_file in json_files:
            ...
            # Store step summary
            experiment_steps_summary[step_number] = step_en
        ...
        for json_file in json_files:
            ...
            if video_path:
                video_path = os.path.join(source_base_dir, video_path)
            else:
                video_path = source_data.get("original_video", "")
            
            # Extract experiment step information
            step_en = source_data.get("step_en", "")
            step_number = source_data.get("step_number", 0)
            
            # Get detailed annotations (if any)
            annotations = source_data.get("annotations", [])

            # Add English title
            experiment_title = source_data.get("title_en", "")

            # Get prompt template based on subject
            assistant_content = get_prompt_template(source_subject, experiment_title)
            
            # If not first step, add previous step summaries
            if step_number > 1:
                assistant_content += "In previous steps:\n"
                for prev_step in range(1, step_number):
                    if prev_step in experiment_steps_summary:
                        assistant_content += f"- Step {prev_step}: {experiment_steps_summary[prev_step]}\n"
                assistant_content += f"\nNow proceeding with Step {step_number}:\n"
            
            gt_content = ""

            # Build output content according to required format
            narration_text = []
            principle_text = []
            safety_text = []
            ...
            if narration_text:
                first_round_content += NARRATION_START
                first_round_content += "\n".join(narration_text) + "\n"
            
            has_safety_or_principle = bool(safety_text or principle_text)
            
            if has_safety_or_principle:
                first_round_content += REFERENCE
                second_round_content = f"{IS_RELEVANT}\n"

                if principle_text:
                    second_round_content += PRINCIPLE_START
                    for note in principle_text:
                        second_round_content += f"{note}\n"
                
                if safety_text:
                    second_round_content += SAFETY_START
                ...
                fine_tuning_item = {
                    "messages": [
                        ...
                        {
                            "role": "user",
                            "content": "<paragraph>passage</paragraph>"
                        },
                        {
                            "role": "assistant",
                            "content": second_round_content.strip()
                        }
                    ],
                    "videos": [video_path],
                ...
                first_round_content += NO_REFERENCE
...
def process_all_datasets(base_source_dir, base_output_dir, experiment_type="kids_fun"):
    """
    Process training, validation and test datasets to generate fine-tuning datasets
    
    Args:
        base_source_dir: base directory containing split/train, split/val, split/test
        base_output_dir: base output directory for fine-tuning datasets
        experiment_type: experiment type
    """
    # Define dataset types
    dataset_types = ["train", "val", "test"]
    
    # Get dataset name from last directory of base_source_dir
    dataset_name = os.path.basename(base_source_dir.rstrip('/'))
    
    for dataset_type in dataset_types:
        # Build source directory and output file paths
        source_dir = os.path.join(base_source_dir, "split", dataset_type)
        
        # Check if source directory exists
        if not os.path.exists(source_dir):
            logging.warning(f"Warning: Source directory {source_dir} does not exist, skipping {dataset_type} dataset")
            continue
        ...
        create_fine_tuning_dataset(source_dir, output_file, experiment_type)
        logging.info(f"Completed processing {dataset_type} dataset!\n")

def main():
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='Create fine-tuning dataset with retrieval markers from processed JSON files')
    ...
    parser.add_argument('--output_base_dir', type=str, 
                        default='data/exp_star_data/subject',
    ...
    # Record total processing information
    all_stats = {"total_items": 0, "processed_units": 0}
    ...
    if args.all:
        ...
        all_stats["processed_units"] += stats["processed_units"]
        ...
        all_stats["total_items"] += items_count
        all_stats["processed_units"] += 1
    else:
